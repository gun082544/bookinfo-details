...
"""
    ...
    PROJECT_KEY = "gun-bookinfo-details"
    PROJECT_NAME = "gun-bookinfo-details"
  }
            ...
            container('helm') {
                script {
                    ...
                    sh "helm template -f k8s/helm-values/values-bookinfo-${ENV_NAME}-details.yaml \
                        ...
                        --namespace gun-bookinfo-${ENV_NAME} gin-details-${ENV_NAME} k8s/helm \
        ...
        container('docker') {
          script {
            ...
            docker.withRegistry('https://ghcr.io', 'gun-ghcr') {
              ...
              docker.build('ghcr.io/gun082544/bookinfo-details:${ENV_NAME}').push()
        ...
        steps {
            container('jnlp') {
                script {
                    ...
                    writeFile file: 'anchore_images' , text: "ghcr.io/gun082544/bookinfo-details:${ENV_NAME}"
          ...
          script {
            ...
            withKubeConfig([credentialsId: 'gke-k8s-kubeconfig']) {
              ...
              sh "helm upgrade -i -f k8s/helm-values/values-bookinfo-${ENV_NAME}-details.yaml --wait \
                --set extraEnv.COMMIT_ID=${scmVars.GIT_COMMIT} \
              --namespace gun-bookinfo-${ENV_NAME} gun-details-${ENV_NAME} k8s/helm"
            } // End withKubeConfig
          } // End script
        } // End container
      } // End steps
